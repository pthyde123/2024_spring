...
subplot_data %>% 
  ...
  mutate(t3_canopy_cm = as.numeric(pmax(`oat_height_7-15-24`,`pea_height_7-15-24`,na.rm=TRUE))) %>% 
  select(block_number,subplot_id,plot_number,subplot_number,t3_canopy_cm,`ManagementFactor:SpringOatPeaIntercrop_2024_NY_biomass_T3`,`oat_height_7-15-24`,`pea_height_7-15-24`) %>% 
...
subplot_data %>% 
  ...
  summarise(median = median(t3_canopy_cm),mean = mean(t3_canopy_cm)) %>% 
