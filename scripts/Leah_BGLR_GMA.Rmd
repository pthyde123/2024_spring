...
Leah: a couple other analyses that would be nice:
Remind me of the year x location combos (environments) this analysis is running over: what are all the trials you have from which you are using data?

2023: NY, IL, SD
2024: NY, IL, SD

For both oat and pea, what are the means for each environment?  I'm assuming there are some wildly differing mean yields for the two crops and the different environments.  It would be good to document that.
...
The analysis of the highest density credible interval you did with the data in "Omega_6.dat" is really nice.  There you are looking at the GMA: Omega_6$VarGMA <- Omega_6$X1 + Omega_6$X3 + 2*Omega_6$X2 . Can you do that analysis also for each of the variances and covariances (i.e., for "X1", "X2", and "X3").  For example, is X2, the covariance significantly negative?  That is, does the credible interval overlap with 0?
...
I'm assuming for both oat and pea, you are analyzing untransformed data.  For the purpose of the grant proposal, I think that's fine.  When we get into wanting to analyze this for publication, we should probably log transform or Box-Cox transform.

see Leah_BGLR_GMA_transformed.Rmd
