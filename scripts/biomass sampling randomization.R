...
Cornell_OatPeaDensity_2024_Ithaca_subplot <- read_excel("~/Intercrop JLJ/2024_spring/Cornell_OatPeaDensity_2024_Ithaca_subplot.xlsx", 
                                                    sheet = "Cornell_OatPeaDensity_2024_Itha")


data <- Cornell_OatPeaDensity_2024_Ithaca_subplot %>% 
  select(subplot_name,subplot_id,block_number,plot_number,subplot_number,biomass_t1,biomass_t2) %>% 
  mutate(plot_subplot = str_c(plot_number,"_",subplot_number)) # create pair_subplot for sorting

set.seed(1236)# make sure you get the same randomization every time


###biomass 1 and 2 not used redone in excel
biomass_1_2 <- data %>%
  mutate(rand = runif(nrow(data))) %>% # add random numbers to arrange by, this isn't reproducible, make permanent here or in excel?
  filter(subplot_number != 2) %>% # remove subplot 2
  arrange(rand) %>% # arrange by pair and then random
  bind_cols(biomass_sample_1_2 = rep(c( 9, 9), times = (52))) %>% # assign sample biomass sample 1 or 2
  select(plot_subplot,biomass_sample_1_2)

set.seed(1237)# make sure you get the same randomization every time
...
biomass_3_4 <- data %>%
  ...
  filter(biomass_t2 != 1 ) %>% 
  arrange(rand) %>% # arrange by pair and then random
  bind_cols(biomass_sample_3_4 = rep(c( 3,4), times = 26)) %>% # assign sample biomass sample 3 or 4
  select(plot_subplot,biomass_sample_3_4)


biomass_sample_management_factor <- 
  data %>% 
  left_join(biomass_1_2, join_by(plot_subplot)) %>% 
  left_join(biomass_3_4, join_by(plot_subplot)) %>% 
  mutate(biomass_1 = if_else(biomass_sample_1_2 == 1,1,NA)) %>% # reassign so each sample is different column / management factor for T3 upload
  mutate(biomass_2 = if_else(biomass_sample_1_2 == 2,1,NA)) %>%
  mutate(biomass_3 = if_else(biomass_sample_3_4 == 3,1,NA)) %>%
  mutate(biomass_4 = if_else(biomass_sample_3_4 == 4,1,NA)) %>% 
...
biomass_sample_management_factor %>% 
  print(n=156)  ### look over


write.csv(biomass_sample_management_factor, file="density_t3_t4_biomass_sample_management_factor.csv", na="", row.names = FALSE)  ## save csv without NA's for T3/OAT
